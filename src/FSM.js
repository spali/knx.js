...
module.exports = machina.Fsm.extend({
  ...
  initialize: function(options) {
    ...
    this.debugPrint(
      util.format('initializing connection to %s (%s)', this.remoteEndpoint
        .addrstring, range));
    switch (range) {
      case 'multicast':
      ...
      case 'loopback':
        ...
        break;
      default:
        throw util.format("IP address % (%s) cannot be used for KNX",
          options.ipAddr, range);
    }
  },
  ...
  states: {
    ...
    connecting: {
      ...
      inbound_CONNECTIONSTATE_RESPONSE: function(datagram) {
        var str = KnxConstants.keyText('RESPONSECODE', datagram.connstate
          .status);
      ...
      "*": function(data) {
        this.debugPrint(util.format('*** deferring Until Transition %j',
          data));
    ...
    disconnecting: {
      _onEnter: function() {
        ...
        this.debugPrint(util.format('connection alive for %d seconds',
          aliveFor / 1000));
        ...
        //
        this.send(this.prepareDatagram(KnxConstants.SERVICE_TYPE.DISCONNECT_REQUEST),
          function(err) {
            // TODO: handle send err
    ...
    requestingConnState: {
      ...
      inbound_CONNECTIONSTATE_RESPONSE: function(datagram) {
        var state = KnxConstants.keyText('RESPONSECODE', datagram.connstate
          .status);
        switch (datagram.connstate.status) {
          ...
          default:
            this.debugPrint(util.format(
              '*** error: %s *** (connstate.code: %d)', state,
      ...
      "*": function(data) {
        this.debugPrint(util.format(
          '*** deferring %s until transition to idle', data.inputType
        ));
        this.deferUntilTransition('idle');
      },
    },
    ...
    sendTunnReq: {
      _onEnter: function(datagram) {
        var sm = this;
        // increment the outgoing sequence counter
        this.seqnum += 1;
    ...
    sendTunnReq_waitACK: {
      ...
      inbound_TUNNELING_ACK: function(datagram) {
        this.debugPrint(util.format(
          '===== datagram %d acknowledged by IP router', datagram.tunnstate
          .seqnum));
        this.transition('idle');
      },
      "*": function(data) {
  ...
  emitEvent: function(datagram) {
    ...
    this.emit(evtName,
      datagram.cemi.src_addr, datagram.cemi.dest_addr, datagram.cemi.apdu
      .data);
    // 4.
    // 'event', 'GroupValue_Write', src, dest, data
    this.emit("event",
      evtName, datagram.cemi.src_addr, datagram.cemi.dest_addr,
